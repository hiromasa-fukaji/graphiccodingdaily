class DesignSystem {
  ...
  setupPane() {
    ...
    // 平体用スケール（X / Y 個別）
    this.pane.addInput(this.selectedParams, 'scaleX', { min: 0.1, max: 5, step: 0.1, label: 'Scale X' });
    this.pane.addInput(this.selectedParams, 'scaleY', { min: 0.1, max: 5, step: 0.1, label: 'Scale Y' });
    this.pane.addInput(this.selectedParams, 'angle', { min: -PI, max: PI, step: 0.01 });
    ...
    this.pane.on('change', (ev) => {
      ...
      if (selectedElement) {
        ...
        // 平体スケールも反映
        selectedElement.scaleX = this.selectedParams.scaleX;
        selectedElement.scaleY = this.selectedParams.scaleY;
  ...
  refreshPane() {
    ...
    if (selectedElement) {
      ...
      // 既存データとの互換性確保（未設定なら1）
      this.selectedParams.scaleX = (typeof selectedElement.scaleX === 'number') ? selectedElement.scaleX : 1;
      this.selectedParams.scaleY = (typeof selectedElement.scaleY === 'number') ? selectedElement.scaleY : 1;
  ...
  render(options = { skipBackground: false }) {
    ...
    for (let element of this.elements) {
      ...
      if (element.isSelected && element.name) {
        ...
        const sx = element.scale * (typeof element.scaleX === 'number' ? element.scaleX : 1);
        const sy = element.scale * (typeof element.scaleY === 'number' ? element.scaleY : 1);
        const halfHeight = element.getHeight() * sy / 2;
    ...
    return {
      ...
      elements: this.elements.map(el => {
        if (el instanceof TextElement) {
          ...
          // 波紋塗り色が設定されていれば保存
          if (el._rippleFillColor) {
            textData.rippleFillColor = el._rippleFillColor;
          }
          return textData;
        } else if (el instanceof ImageElement) {
          // 画像をBase64に変換
  ...
  importJSON(data) {
    ...
    const loadPromises = data.elements.map(elementData => {
      if (elementData.type === 'text') {
        ...
        el.scaleX = (typeof elementData.scaleX === 'number') ? elementData.scaleX : 1;
        el.scaleY = (typeof elementData.scaleY === 'number') ? elementData.scaleY : 1;
        el.visible = (elementData.visible !== false); // 存在しなければtrue
        // 波紋塗り色を復元
        if (elementData.rippleFillColor) {
          el._rippleFillColor = elementData.rippleFillColor;
        }
    ...
    for (let el of this.elements) {
      try {
        if (el instanceof TextElement) {
          ...
          const sx = el.scale * (typeof el.scaleX === 'number' ? el.scaleX : 1);
          const sy = el.scale * (typeof el.scaleY === 'number' ? el.scaleY : 1);
          const transform = `translate(${el.x},${el.y}) rotate(${degrees(el.angle)}) scale(${sx} ${sy})`;
          ...
          const sx = el.scale * (typeof el.scaleX === 'number' ? el.scaleX : 1);
          const sy = el.scale * (typeof el.scaleY === 'number' ? el.scaleY : 1);
          const tx = el.x - el.width * sx / 2;
          const ty = el.y - el.height * sy / 2;
          const cx = el.width * sx / 2;
          const cy = el.height * sy / 2;
          const transform = `translate(${tx},${ty}) rotate(${degrees(el.angle)} ${cx} ${cy}) scale(${sx} ${sy})`;
